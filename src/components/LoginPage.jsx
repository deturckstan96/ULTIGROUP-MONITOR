...
export default function LoginPage({ fout = null }) {
  const [email,   setEmail]   = useState('stan@ultigroup.be')
  ...
  async function handleLogin(e) {
    ...
    if (error) {
      setErr('Fout e-mail of wachtwoord')
    }
    // Altijd loading uitzetten — onAuthStateChange handelt de rest
    setLoading(false)
  }

  return (
    <div style={{
      position: 'fixed', inset: 0, background: '#0f2748',
      ...
      <form onSubmit={handleLogin} style={{ width: '100%', maxWidth: 320, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <button
          ...
          disabled={loading}
          style={{
            padding: 13, borderRadius: 10, background: loading ? '#1d4ed8' : '#2563eb',
